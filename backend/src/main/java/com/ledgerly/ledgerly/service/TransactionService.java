...
public class TransactionService {
    ...
    public void processTransaction(Transaction request, String idempotencyKey) {
        ...
        // 6. Attach the FULL account entities (not just the shallow {id} objects from the request)
        request.setFromAccount(sender);
        request.setToAccount(receiver);

        // 7. Save the transaction itself
        transactionRepository.save(request);
    }

    public List<Transaction> fetchUserTransaction(String email, Integer accountId) {
        accountRepository.findByIdAndUserEmail(accountId, email)
                .orElseThrow(() -> new RuntimeException("Account not found or access denied"));

        return transactionRepository.findTransactionsByUserEmailAndAccountId(email, accountId);
    }
